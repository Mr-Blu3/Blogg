 ...
 */
	...
	*/
/*
	wp_enqueue_script( 'react', get_template_directory_uri() . 'https://fb.me/react-15.2.1.js', array('react') );
	wp_enqueue_script( 'react', get_template_directory_uri() . 'https://fb.me/react-dom-15.2.1.js', array('react') );
	wp_enqueue_script( 'react', get_template_directory_uri() . 'js/libs/browser.min.js', array('react') ); */
...
//ToDo: Make type="text/babel" on script tag
...
function barletta_widgets_init() {
	...
	register_sidebar( array(
		'name'          => __( 'Header Widget', 'barletta' ),
		'id'            => 'header-widget',
		'description'   => __( 'Main header bar', 'barletta' ),
		'before_widget' => '<div id="%1$s" class="widget-header %2$s">',
		'after_widget'  => '</div>',
		'before_title'  => '<div class="widget-header-title"><span>',
		'after_title'   => '</span></div>',
	) );

	register_sidebar( array(
		'name'          => __( 'Intro widget', 'barletta' ),
		'id'            => 'intro-widget',
		'description'   => __( 'Main intro', 'barletta' ),
		'before_widget' => '<div id="%1$s" class="widget-intro %2$s">',
		'after_widget'  => '</div>',
		'before_title'  => "<div class='widget-intro-title'><br /><span>",
		'after_title'   => '</span></div>',
	) );
...
function barletta_my_search_form($text) {
	return str_replace('value="Search"', 'value=""', $text);
}
...
function insertTopic($httpUrl) {

	$sTarget_dir = realpath(__DIR__ . '/../../')."/images/pontusUploadedFiles/";
	$sTarget_file = $sTarget_dir . basename($_FILES["files"]["name"]);
	$bUploadOk = true;
	
	if(isset($_POST["btn_topic"])) {		
		...
		if ($bUploadOk) {
		    move_uploaded_file(
	    		$_FILES["files"]["tmp_name"], $sTarget_file
    		);
		}
		...
		wp_insert_post([
			...
			"<img src='".$httpUrl.$_FILES["files"]["name"]."' 
			/>",
			'post_parent' => 1,
			'post_status' => 'publish',
			'post_name' => htmlentities($_POST['title'])
		], true);
